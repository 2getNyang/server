...
public class BoardEsController {
    ...
    public ResponseEntity<ApiResponse<Page<? extends BoardListDTO>>> searchReviewBoard(
    ...
    public ResponseEntity<ApiResponse<Page<? extends BoardListDTO>>> searchSnsBoard(
            @Parameter(description = "검색어", example = "제목") @RequestParam String keyword,
            @Parameter(description = "페이지 번호", example = "0")@RequestParam(defaultValue = "0") int page,
            @Parameter(description = "한 페이지에 보여줄 게시물 개수", example = "12") @RequestParam(defaultValue = "12") int size) {
    ...
    @GetMapping("/lost/elasticsearch")
    public ResponseEntity<ApiResponse<Page<? extends BoardListDTO>>> searchLostBoard(
